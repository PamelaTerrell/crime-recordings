...
export default function PublicMediaPlayer({
  ...
  if (playbackUrl) {
        ...
        <video
          ...
          className={
            featured
              ...
              : "max-h-[75vh] w-full border border-white/10 bg-black"
          }
        >
    ...
    return (
      <div className="border border-white/10 bg-[#10151b] p-4">
        <audio
          controls
          preload="metadata"
          src={playbackUrl}
  ...
  if (featured) {
    return (
      <div className="relative grid min-h-[62vh] place-items-center overflow-hidden border-y border-white/10 bg-black px-6 py-20 text-center md:min-h-[78vh]">
        <div
          aria-hidden="true"
  ...
  return (
    <div className="border-t border-white/10 pt-5">
      {isMemberOnly ? (
        <p className="mb-4 max-w-2xl text-sm leading-6 text-[#a8adb5]">
          Active Crime Recordings members can securely play
      ...
      <div className="flex flex-wrap items-center gap-4">
        <button
          ...
          onClick={loadPlayer}
          disabled={pending}
          className="inline-flex min-h-12 items-center justify-center border border-[#c8a66a] bg-[#c8a66a] px-6 text-xs font-extrabold uppercase tracking-[0.1em] text-[#111318] transition hover:bg-[#e1c58f] disabled:cursor-wait disabled:opacity-60"
        >
          {pending
            ? "Checking access…"
            : isVideo
        ...
        <span className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#747b84]">
          {isMemberOnly
            ? "Members only"
            : "Available now"}
        </span>
      ...
      {requiresSignIn ? (
        <Link
          href="/login"
          className="mt-5 inline-flex min-h-12 items-center justify-center border border-white/20 px-6 text-xs font-extrabold uppercase tracking-[0.1em] text-[#f4f1e9] transition hover:border-[#c8a66a] hover:text-[#e1c58f]"
        >
          Sign in
        </Link>
      ) : null}
      ...
      {requiresMembership ? (
        <Link
          href="/membership"
          className="mt-5 inline-flex min-h-12 items-center justify-center border border-[#c8a66a] px-6 text-xs font-extrabold uppercase tracking-[0.1em] text-[#e1c58f] transition hover:bg-[#c8a66a]/10"
        >
          Become a member
        </Link>
      ) : null}
